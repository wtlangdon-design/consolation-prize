...
AMBIENT = 0.52         # night: everything starts well under-lit
...
def compose(with_coach: bool = True, lamp_x: int | None = None) -> tuple[IndexedCanvas, Palette]:
    ...
    for y in range(HORIZON + 2, HEIGHT):
        ...
        for x in range(WIDTH):
            dither_pixel(canvas, x, y, grey, max(0.03, 0.16 + 0.10 * walk), BAYER2)

    # The road itself: churned mud, slightly paler than the verge.
    for y in range(ROAD_Y, HEIGHT):
        walk = (y - ROAD_Y) / max(1, HEIGHT - ROAD_Y)
        for x in range(WIDTH):
            dither_pixel(canvas, x, y, mud, max(0.04, 0.20 + 0.16 * walk), BAYER2)
    for _ in range(90):
        y = rng.randrange(ROAD_Y, HEIGHT)
        canvas.hline(rng.randrange(0, WIDTH), y, 2 + rng.randrange(0, 8),
                     mud.frac(0.12 + 0.10 * rng.random()))
    ...
    if with_coach:
        field.add_lamp(Lamp(x=COACH_X - 16, y=ROAD_Y - 4, radius=30, intensity=0.44, squash=1.2))
...
def town_sign(canvas: IndexedCanvas, palette: Palette, x: int, y: int, post, board) -> None:
    ...
    # Four repaintings, each a slightly different patch on the lower half.
    for index, (px, pw) in enumerate(((4, 9), (7, 7), (10, 6), (12, 5))):
        canvas.rect(x + px, y + 8, pw, 4, board.frac(0.10 + 0.04 * index))
...
def coach(canvas: IndexedCanvas, palette: Palette, rng, x: int, y: int, body, iron, brass) -> None:
    ...
    canvas.put(x - 16, y - 4, brass.frac(0.60))             # the coach lantern
    canvas.put(x - 16, y - 5, brass.at(brass.count - 2))
