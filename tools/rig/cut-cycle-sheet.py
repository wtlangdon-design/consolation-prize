...
sheet = np.array(Image.open(args.sheet).convert("RGB")).astype(int)
...
poses = [int(p) for p in args.poses.split(",")]
if max(poses) > len(comps):
    raise SystemExit(f"the sheet has {len(comps)} figures; pose {max(poses)} does not exist")
...
for old in out.glob("walk-*.png"):
    old.unlink()
record = []
for i, (p, k) in enumerate(zip(poses, keyed)):
    ...
    path = out / f"walk-{i:02d}.png"
    Image.fromarray(np.clip(canvas, 0, 255).astype(np.uint8), "RGBA").save(path)
    ca = canvas[..., 3] > 128
    record.append({"frame": path.name, "pose": p, "sheetHeight": heights[i], "sheetWidth": widths[i], "placedHeight": int(ca.any(1).sum()), "placedWidth": int(ca.any(0).sum()), "soleRow": int(np.nonzero(ca.any(1))[0].max()), "torsoShift": dx})
min_h = min(r["placedHeight"] for r in record)
meta = dict(method="authored-cycle-sheet", source=args.sheet, sourceSha256=hashlib.sha256(Path(args.sheet).read_bytes()).hexdigest(), key=args.key,
...
print(f"cut {len(poses)} whole figures from {args.sheet}: sheet heights {heights}, scale x{factor:.4f}, placed heights {[r['placedHeight'] for r in record]}, torso shifts {[r['torsoShift'] for r in record]}, soles {[r['soleRow'] for r in record]}")
