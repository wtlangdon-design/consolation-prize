...
| P2b | Three variants restated word-for-word inside variant 1 | `thads_room/window` LISTEN 2 and 3, `undertaker/jar_of_teeth` LOOK 3 | Doc 08 and doc 09 pack two or three sentences into variant 1 and doc 26's repeats pull them back out, so a third look gives the back half of the first. Reported by check-examine-lines as notes, not failures — a restatement can be a callback, and the tool does not decide which |
...
| P5 | **A carried lamp cannot move.** The engine composites sprites OVER a finished background and cannot modify what is under one, so a lamp's ground pool is fixed where it is painted | Room 1, and any room with a hand-held or moved light | **Not a Room 1 defect — a known deferred engine capability, scoped here so nobody meets it by surprise in Act III.** Measured: `compose(lamp_x=140)` moves 5,293 pixels across 262 columns. The pool is not decoration on the ground, it IS the ground for a third of the frame's width. Hob does not move (ruled — doc 17 gives him one crossing and never returns him), so Room 1 ships correct and this blocks nothing. **The maths is already done if it is ever wanted:** the pool fits `L(ρ) = 27 + 97/(1 + (ρ/6.2)²)` with `ρ = √(((x−86)/2.4)² + (y−107)²)`, residual under 3 luminance across ρ 3–11, fitted in VALUE space rather than linear light — which is the space the lighting pass already steps in, so it transfers without conversion. What it needs is somewhere to run: a runtime radial pass over the background before sprites, fed by the `lightSources` block every room already declares and nothing currently reads. That one feature would serve every lamp, stove, doorway and window in the game, which is the argument for building it once rather than per room |
