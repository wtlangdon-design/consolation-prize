...
## Q109 · Room 5 Act I gameplay acceptance pass — three defects found and fixed at source; awaiting Tyler's gameplay acceptance
...
- **R5-C padded log ready** — the padded log is not an item; C2-C4 (Fanshawe's office, the funeral window) are not built; the confrontation's trigger is UNSPECIFIED (doc 02 C5 is "Show Winnie", doc 04 gives WIN_B2 a scripted opening and no row or USE that reaches it); nothing in data routes to WIN_B2; `T_QUEUE_PADDED` has no documented writer. A fixture would have to invent the action it exists to test.
- **R5-D second ledger** — needs C5 consumed, which no flag records; WIN_B3 is not extracted; the documents disagree whether the ledger's hiding place is Room 5 (doc 05:155,169) or Room 6 (doc 08:128-145); whether OPEN on the board ever succeeds, and whether she leaves the counter, is unspecified (doc 25:47, 122). Q109 lists all of this.
- **R5-E assay / Document B** — C6 requires C5; Document B is not an item; what Thad hands over is unspecified (doc 02 C6's requirement is "C5" alone); the assay is collected off screen (doc 04:176 makes WIN_F1's the first the player watches); `ASSAY_PAIRS` is not declared.
- **R5-F false death certificate** — WIN_C1 is not extracted; the certificate is not an item; E4's flag is unspecified; ACT 3's writer (S2, the funeral) is not built; E0-E3 are not built.
