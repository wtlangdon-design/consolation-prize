...
- **R5-C padded log ready** — needs: the `padded_log` item (errata 66 A; no examine copy authored); C2–C4 built; USE-item-on-Winnie resolution; the opener that starts WIN_B2 from the evidence action (errata 66 C). The trigger is now ruled; the runtime for it is not built.
