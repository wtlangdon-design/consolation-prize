...
## Q121 · Opening-set retrofit, phase 1.5E: Main Street's board and trough repaired as structure and made plate (2 operations), the Nugget owner-accepted and frozen — **BUILT 2026-09-06, FINAL STRUCTURAL REVIEW AWAITING TYLER**

Tyler's review of Phase 1.5D (Q120): the notice-board area still looked strangely merged into the neighbouring architecture, the chapel front near it possibly incomplete or damaged by the stitch history, and the trough and hitching rail an obvious visual mess — two independently authored pieces overlapping without a physical relationship. Rescaling or repositioning the overlays is not the solution. His ruling changed the strategy: the board's structure and the trough are permanent scenery and belong in the environment; only the notice paper that changes stays stateful (doc 35's own classification: the trough PLATE, the notices STATEFUL for their content). Two local STRUCTURAL repairs were authorized, one per region, one operation each, no retries. **The Nugget's corrected environment is accepted and frozen.**

**Preflight diagnosis (before any generation).** The candidate plate is the west and east source panels stitched at x 1704–1800 with a 96 px feather, and the west panel's freestanding notice board (x 1615–1790) fell inside that feather: its right half was a ghost — a doubled lantern at about (1755, 430), a half-transparent post, its roof dissolving into the east panel's porch roof at the same height. That is what read as a board fused into a building. **The church is not structurally broken.** Its steeple, gable and both walls match the west source; its right wall ends at x ≈ 1690 with open hills between it and the east storefront's false front (x ≈ 1755); a nearer building in front of a distant church is legitimate occlusion. Only its lower right wall behind the board sat in the seam. So Region A's mask freed the board, the church's lower right wall behind it and the east porch's ghosted left end (x 1585–1800, y 335–615) and kept the steeple, the gable, the Company porch, the lit window and the mud. Region B: the 1.5D trough (154 px tall) stood with its back rim above the top of the east rail's near post while its end passed behind the post, so the post read as rising out of the trough; the two had been authored apart. Its mask freed the mud in front of the saloon from the boardwalk foot down with the whole east rail inside it (x 1960–2690, y 600–864), so trough and rail were authored together; the 1.5D trough result rode along as the object reference only, and the pasted trough was never in the source.

**Region A (1 of 1, used, kept).** On a 520×520 window scaled ×1.96 into a 1024 canvas with the problem at the centre (`tools/retrofit/phase15e-prep.py`, `art/staging/room-02/repair-a/`), the model was asked to repair the section as one coherent piece of frontier architecture. Result: the church's lower right wall continues down behind the board as plain clapboard; the board is a modest freestanding board of rough timber on two posts standing on the boardwalk, a plank cap, a handful of ordinary pinned papers, with clear night air between it and the storefront's porch; the porch's left end is finished as a plain roof end and post; the lit window is untouched. Out-of-mask drift 2.4 mean absolute levels; nothing outside the zone was taken.

**Region B (1 of 1, used, kept).** On an 880×484 window scaled ×1.16 and letterboxed at the middle of a 1024 canvas (`art/staging/room-02/repair-b/`), the model was asked for one physical scene. Result: the hitching rail kept as two rough posts and a bar, chest-high, standing in the mud; a low, long rough-plank trough (x 1980–2232, front base ≈ 798, about a quarter of a man's height at its face) on the mud beside the rail's west end with a stride of open mud between them, a few inches of dark still water catching the saloon's light; mud continuous with the plate, contact shadows under both. Nothing passes through anything.

**Integration (0 operations).** Each result's masked zone, scaled back 1:1, was composited INTO a new plate, `art/staging/room-02/street-candidate-03/candidate-plate.png`, with an 8 px feather inside the zone (`tools/retrofit/phase15e-integrate.py`, `integrate.json`). The 1.5D overlays are retired (REJECTED in the contract). State architecture now: `posted_notices` ORDINARY draws nothing — the plate is the board with its papers; FUNERAL is a sheet-only full-frame overlay, the Phase 1.5B aged sheet pinned centre-top on the same board's face (no board pixels in it, no wording invented). `water_trough` has no state image; it is scenery. Its occlusion and the rail's are the room's plane-1 mask, an authored silhouette of the trough and the rail's two posts and bar (`plane-1-silhouettes.json`); mud_far AND mud_mid are plane 1, since both objects' bases (798; 790 and 810) lie below mud_mid's foot (776). The trough's obstacle is its near footprint only (y 776–802): the compiler carves a band's whole column under an obstacle, so an obstacle reaching into mud_mid would remove the ground behind the trough altogether. The rail's obstacle carves the column under and behind it to y 815. The hotspot rect starts at the trough's body (y 700), leaving the ground directly behind its far rim clickable. The Company sign: weathered in the normal state, gilt in Act III, both verified live; the underlying board stays blank plate.

**Proof.** `check-occlusion` PASS with two proof points (behind the trough on mud_mid; behind the rail on mud_far); live routes `main-street` and `main-street-act3` PASS: Thad in front of the trough and behind it, the ordinary board and the funeral sheet, the weathered and gilt sign. Sanity pass at 1:1, 2× and 3× over the board, the storefront, the church front, the sign, the trough, the rail, the dog zone, the porches, the boardwalk and both patch boundaries: no seam, no duplicated edge, no discontinuity (`renders/opening-set-retrofit/phase15e-*.webp`).

**The Nugget.** OWNER ACCEPTED AND FROZEN (Tyler, 2026-09-06): `NUGGET ENVIRONMENT — OWNER ACCEPTED` in its contract, the corrected-03 plate at ACCEPTED-NOT-SHIPPING (staged, not promoted). Nothing of it changed in this phase. Phase 2 owns Thad's depth scale, the nine patrons, room life, flicker and actor integration.

**Contracts.** Main Street `MAIN STREET STRUCTURAL ENVIRONMENT — READY FOR OWNER FINAL REVIEW`; not owner-accepted, not promoted, the shipping room untouched. No character, no room life, no flicker, no Thad recalibration, no Room 6.

**PHASE 2 HAS NOT BEGUN. NUGGET IS OWNER-ACCEPTED. MAIN STREET AWAITS FINAL STRUCTURAL ENVIRONMENT REVIEW.**

## Q122 · Opening-set retrofit, phase 1.5F: Main Street's final micro-corrections — the sign aligned, the chapel front given its architecture (1 operation), the rail given ground behind it — **BUILT 2026-09-06, FINAL ENVIRONMENT ACCEPTANCE AWAITING TYLER**

Tyler's live review of Phase 1.5E (Q121) was strongly positive: the notice board, the water trough, the trough/rail layout and the street as a whole are substantially correct and are frozen, as is the Nugget. Three narrow issues remained, and this phase is those three and nothing else.

**THE SIGN (0 image operations).** The complaint was that `CONSOLATION IMPROVEMENT COMPANY` reads crooked against a frontal facade. Measured before touching anything: **the plate's physical board is level** — its inner shadow lines lie at y 142 and y 226 at every column from x 360 to x 700, its inner side shadows at x 344 and x 704. Two things in the lettering layer were not. The type carried **+1.43°** of slant (line 1 cap-top +1.20, baseline +1.80; line 2 +1.29 and +1.41), inherited from `art/objects/room-02/company-sign-gilt.png` — the one authored rendering of the words, which is a crop of the SHIPPING plate's sign, whose board was drawn at an angle. And the layer carried that crop's **wood**: Phase 1.5 derived its alpha as a luminance ramp `(lum − 60) / 50`, which took every wooden pixel brighter than 60 at partial alpha — 6,219 translucent pixels against 3,594 solid letter ones, a slanted translucent rectangle of the old board sitting on the new level one. The correction lifts the ramp to 95–125 (at luminance 85 the source's edge rows are already empty, so no wood survives), de-rotates the letters by their own measured slant to a residual of 0.05°, and centres them on the board's inner face with margins of 32/32 and 9/9. Both states are cut from that one corrected lettering, so weathered and gilt cannot drift apart. `tools/retrofit/phase15f-sign.py`, `art/staging/room-02/companions-03/sign-align.json`.

**THE CHAPEL (1 image operation, conditional and proven).** The brief required the cause to be proven with the board out of the way before any repaint. The board is plate, so the diagnostic clones the church's own clapboard across its footprint (`tools/retrofit/phase15f-chapel-diagnose.py`, `renders/opening-set-retrofit/phase15f-chapel-diagnostic.webp`). With it gone: the steeple, gable and roof are one structure, the nave continues left behind the Improvement Company, the wall runs unbroken to the boardwalk, and there is **no stitch artifact and no truncation**. What there is, is **no architecture at all** — no door, no window, no step, no plinth anywhere on the front. A church nobody can enter, whose facade is a blank slab meeting a boardwalk; at gameplay scale a steeple and a triangle rather than a building, which is the failure §10 names. Moving the board could not fix that (it would only uncover more blank wall), so this is the brief's CASE 2 and the conditional operation was spent. One tightly masked repair on a local canvas with the chapel centred (`art/staging/room-02/chapel-01/chapel-op.json`, zone x 1505–1740, y 330–578) gave the front a double door with a lintel, two or three plain steps, a small window on each side and a base where the wall meets the ground, in the same night value — the wall's mean luminance moved 21.1 → 22.8. The steeple, the roof, the Company, the storefront, the mountains and the boardwalk are untouched. **The board is frozen and was put back exactly**: its own pixels, cut from the pre-phase plate by the wood-hue silhouette that separates warm board timber from cool clapboard, dilated a pixel and composited over the result; where the model painted its own board outside that silhouette, the cover is the chapel's siding by per-row median (the siding is horizontal courses, so a row median carries no feature to repeat), and everything right of the board's edge is simply the plate as it was. `tools/retrofit/phase15f-chapel-integrate.py`.

**THE RAIL (0 image operations).** Actors could not stand behind the east hitching rail. The cause was the obstacle, not the art: `rail_east` was 115 px deep from y 700, and the compiler carves a band's whole column under an obstacle, so mud_mid lost everything between x 2270 and 2615 — the entire band behind the rail — and the only ground near the rail was its own line. The obstacle is now the rail's own ground line, y 776–816 across x 2270–2615, measured off the posts' feet at 6× (near post base y ≈ 786 at x 2290–2312, far post ≈ 812 at x 2550–2578). Behind it, mud_far (600–688) and mud_mid (688–776) are unbroken walkable ground at plane 1; in front, mud_near below y 816 is plane 0. Depth therefore follows the **foot baseline**, which is how the engine already resolves it — the walk box under an actor's feet names the plane, and the plane-1 mask holds the trough and the rail. Nothing is hard-coded for Thad: the pie woman stands on the far mud at 2450,662 and the rail draws over her by the same rule. He passes from front to behind round the posts, through the 38 px gap between the trough and the near post or past the far end, and never through a post. Live proof route `main-street-rail`, six positions and the walks between them: in front mid-span (clip 0), behind mid-span (clip 1), behind the near post, beside the far post, asked for the ground the far post stands on (he stops behind it, never inside it), and out in front again — `renders/proofs/candidates/main-street-rail/life.json`, `renders/opening-set-retrofit/phase15f-rail.webp`.

**Frozen and untouched.** The board's art and papers, the trough, the trough/rail structural patch, the rail's art, the dog, the plate's composition, the buildings, the mountains, the road, the mud, the night mood and the practical lights. The Nugget: OWNER-ACCEPTED AND UNCHANGED. The legacy human art (map seller, pie woman, letter-writer) stays a Phase 2 subject and was not recast or rescaled; the pie woman appears only as the generic-actor proof the brief asks for.

**Contract.** `ENVIRONMENT — FINAL MICRO-CORRECTIONS COMPLETE / AWAITING OWNER ACCEPTANCE`. Nothing promoted; the shipping room untouched. No character generation, no room life, no flicker, no Room 6.

**PHASE 2 HAS NOT BEGUN. NUGGET IS OWNER-ACCEPTED. MAIN STREET AWAITS FINAL OWNER ENVIRONMENT RULING.**

## Q123 · Opening-set retrofit, phase 1.5G: Main Street's runtime — the rail as a barrier, the trough's mask cut from the trough, the sign rebuilt letter by letter (0 image operations) — **BUILT 2026-09-06, RUNTIME ACCEPTANCE AWAITING TYLER**

Tyler's live review of Phase 1.5F passed the structural art — chapel, notice board, trough, hitching rail, street — and froze it. Three defects left, all runtime, all fixed here with **no image operation and no pixel of the environment changed**.

**THE RAIL. He could stand on it.** The bar is drawn across y 640–682 and the far mud was walkable at those depths, so a click on the rail put his FEET where the bar is drawn: a man standing on a fence. The obstacle was on the posts' feet only. The rail is now a barrier on **both** of its own lines — the bar's drawn line (y 636–690) and the posts' footing (776–816, their feet measured at 786 and 812) — with real walkable ground between them (mud_mid, 690–776, where the bar crosses a man's shins as it should) and in front (from 816). Crossing at mid-span is impossible: the walk-box graph routes him round an end, which the proof catches mid-walk. A click on the bar itself resolves to the nearest ground beyond it. That required one compiler change, `keepsGroundAbove`: carving a band used to drop everything above the obstacle as well as under it, which is exactly why an obstacle on an object's own line used to take away the ground behind the object. It is opt-in, so no other room's geometry moves.

**THE TROUGH. It clipped a man standing clear of it.** The Phase 1.5E occluder was a six-point polygon drawn around the trough, and a six-point polygon around a box in oblique projection is a convex hull: it swallowed a wedge of mud above the far rim and the contact shadow below the near wall — **2,995 pixels of mud**, which is what ate a man's shins when he stood beside the trough. The mask is now **cut from the plate's own pixels** (`tools/retrofit/phase15g-masks.py`): the water and the dark front boards and the lit rim against a per-row median of the mud beside them, closed, filled, largest component, then everything below the trough's own base line dropped — a contact shadow is ground, not an occluder. 18,800 px against 22,576, every one inside the object, bbox 1976,677–2234,801. The rail keeps authored quads, because the rail is three straight members and a quad is exactly what each one is; segmenting there swallowed half the street, since the mud beside it carries the saloon's light. One file, one source: the room declares a single plane, and nothing else masks anything near the trough. The trough's obstacle moved onto its base line too, which removes the wedge where a man stood in front of the trough and the walk band still called him behind it.

**THE SIGN. Level is not straight.** 1.5F rotated the whole crop by its measured slant and reported a residual of 0.05° — and Tyler, looking at the live room, still called it crooked. He was right, and the metric was not wrong: rotating a raster levels the BLOCK and leaves every letter where the old angled board put it, a pixel up and a pixel down along the line. A stepping line is what an eye reads as crooked, and no angle measurement can see it. The lettering is now **rebuilt from the authored letterforms**: the glyphs are cut on their own luminance ramp, the two lines separated by their own baselines, each line's slant measured by Theil–Sen over the glyph bottoms, and each glyph MOVED by whole pixels — never rotated, never resampled — onto its line's own row. Baseline spread: 6 px → 0 on CONSOLATION, 7 px → 0 on IMPROVEMENT COMPANY. Both states are cut from the one raster, so weathered and gilt cannot differ by a pixel in position or angle.

**Two new validators, both of which fail on the defect they were written for.** `check-occluder-bounds` holds a mask inside the geometry its cut record claims (it rejects the 1.5E polygon by 2,995 px and passes the new mask); `check-sign-baselines` measures the shipping raster's glyph feet and refuses a line whose letters step, and refuses two states that are not the same geometry. The suite is 55 checks now.

**Proofs, live.** `main-street-rail`: in front mid-span (plane 0, draws over the bar), asked to cross mid-span (round the west end, caught mid-walk), behind mid-span (plane 1, the bar over him), clicked on the rail itself (stops on the ground beyond it), behind the near post, round the far end. `main-street-trough`: the mask in green over the plate, behind-centre (only what the trough covers is gone), left and right at the same depth (whole), in front (draws over), just outside the east end (whole), and the position that used to clip him — which is inside the trough's hotspot, so it is an ordinary LOOK AT and he walks to the trough's own approach point. The pie woman's placement moved from y 662 to 706 for the same reason as Thad's: 662 is where the bar is drawn. Her art is untouched and stays a Phase 2 subject.

**Frozen and untouched.** Every pixel of the Main Street plate, the chapel, the notice board, the trough, the rail, the dog, the buildings, the lighting, the mud and the boardwalk. The Nugget: OWNER-ACCEPTED AND UNCHANGED.

**Contract.** `MAIN STREET RUNTIME GEOMETRY CORRECTIONS — AWAITING OWNER VISUAL/GAMEPLAY ACCEPTANCE`. Nothing promoted; the shipping room untouched. No characters, no room life, no flicker, no Room 6.

**PHASE 2 HAS NOT BEGUN. NUGGET IS OWNER-ACCEPTED. MAIN STREET AWAITS FINAL OWNER REVIEW OF RAIL COLLISION, TROUGH OCCLUSION, AND SIGN ALIGNMENT.**

## Q124 · Opening-set retrofit, phase 1.5H: the hitching rail made a fence — one continuous no-cross barrier, and a test that walks through it to prove it — **BUILT 2026-09-06, NAVIGATION ACCEPTANCE AWAITING TYLER**

Tyler's manual review of Phase 1.5G left one Main Street problem: **Thad could still walk through the hitching rail**, front to back and back to front. Everything else — plate, chapel, board, sign, trough, trough occlusion, rail art, rail occlusion, dog, lighting — passed and is frozen. This is navigation topology only, **0 image operations**, and no pixel of the environment changed.

**THE ROOT CAUSE, and why 1.5G's proof missed it.** 1.5G proved the rail with a scripted route and a screenshot at each end of the walk. A screenshot at each end cannot see the walk: the actor follows the waypoints the router hands him, and the question is whether that POLYLINE crosses the fence. It did. The footing obstacle ran x 2270–2615 — starting at the bar's own west tip — and the trough's obstacle ended at x 2236, leaving a **34 px gap between the two**. `WalkBoxes.route` picks each crossing point NEAREST THE DESTINATION, so for any destination inside the rail's span the crossing landed at **x 2268: the exact tip of the bar**. On screen that is a man walking through the end of the fence. Sampling the live walks proves it: on the 1.5G geometry, **all ten of ten** attempts crossed, every one of them at x 2268–2276.

**THE BARRIER.** One continuous strip on the fence's own ground line, from the trough's east edge to 40 px clear of the bar's east tip: **x 2236–2652, y 776–816** — the whole gap between the ground behind the fence (mud_mid, down to y 776) and the ground in front (mud_near, from y 816). The posts' feet (786 at x 2290–2320, 812 at x 2555–2583) both lie inside it, which is why one flat strip describes a fence that recedes. It is 40 px deep and blocks nothing else: behind the rail, mud_mid_3 runs the whole span (2268–2614, y 690–776) and in front mud_near_2 runs 2236–2652 at y 816–864, both continuous with the rest of the street. The only front/back crossings left are **round the east end at x 2652**, clear of the rail, and **west of the trough at x 1976** — the west end being genuinely blocked by the trough, which the brief allows.

**THE CONTRACT IS NOW DATA.** The room declares `navigation.barriers`: the strip, the separator line (the strip's middle), the span in which no side change is allowed, and ground on each side to stand on. `tools/gauntlet/rail-crossing.mjs` drives the live candidate like a player — five x positions across the span each way, plus six clicks on the fence itself (the bar, under it, over it, the gap beneath, each post) — samples his feet every 120 ms for the whole walk, and asserts that **the side only ever changes outside the span**, and that no click leaves him standing inside the footing. Ten crossings and six clicks pass; the same test fails on the 1.5G geometry. Every sampled path is drawn over the plate in `renders/opening-set-retrofit/phase15h-rail-navigation.webp`, which is the sheet to look at.

**Frozen.** The plate, the chapel, the notice board, the company sign, the trough and its mask, the rail art and its occlusion mask, the dog, the lighting, the buildings, the mud, the boardwalk — untouched, byte for byte. No human placement or art changed. The Nugget: OWNER-ACCEPTED AND UNCHANGED.

**Contract.** `MAIN STREET HITCHING-RAIL NAVIGATION — AWAITING OWNER FINAL ACCEPTANCE`. Nothing promoted; the shipping room untouched.

**PHASE 2 HAS NOT BEGUN. NUGGET IS OWNER-ACCEPTED. MAIN STREET HAS ONE REMAINING OWNER GATE: HITCHING-RAIL NO-CROSS NAVIGATION.**

---

## Q125 · Opening-set retrofit, phase 1.5I: front-only hitching-rail navigation — the ground behind the rail retired — **BUILT 2026-09-06, NAVIGATION ACCEPTANCE AWAITING TYLER**

Tyler manually tested 1.5H and it still failed. Despite an automated proof reporting no crossing, he could put Thad onto and through the hitching-rail geometry in the live deployed room.

**OWNER RULING.** Stop preserving walkability behind this rail. Thad may walk only on its **front / near side**. He may move around the rail's ends where the street is genuinely open. He may not walk behind it, cross it in either direction, stand with his feet on either post, stand with his baseline on the bar, or occupy any point that visually places his body through the rail. This supersedes the requirement to support a walkable rear side with occlusion. The rail is now a foreground environmental boundary. It is the final fallback because three attempts at dual-side navigation have failed owner live review.

**WHY 1.5H STILL ALLOWED IT, and why every previous proof agreed with itself.** 1.5F gave the rail walkable ground behind it. 1.5G put the obstacle on the fence's own line. 1.5H made the fence uncrossable between its ends and proved it by sampling the live walk rather than its endpoints. Each fixed the defect it was looking at, and each was asking the same wrong question — *can he cross it* — when the question was *may he be there at all*. The strip behind the rail is `mud_mid_3`, x 2268–2614, y 690–776. That is exactly the depth band the bar is drawn across. **A man standing on it is a man standing in the fence, whatever route he took to get there**, and no crossing test can see that, because no crossing occurred. 1.5H's own test would have kept passing forever.

**REPRODUCED FIRST, on the complete 1.5H build, before anything was changed.** The new click test run against that geometry and that engine fails **31 ways**, and puts him behind or on the rail on **nine of nine** clicks: on the bar at 2443,636 · on the near post at 2305,699 · on the far post at 2568,769 · behind mid-span at 2443,729, 2443,699, 2339,729, 2339,759, 2548,729, 2548,699.

**THE GEOMETRY.** `rail_east` stops being a barrier and becomes the rear region: **x 2236 → 2652, y 600 → 816** — from the trough's east edge to 40 px clear of the bar's east tip, from the top of the far mud to the top of the front band. The compiler **carves** it, so no walk box exists behind the rail; this is not an overlay for the router to route around, and there is no rear side left to cross to. `rail_east_bar` is retired with the ground it existed to keep. A second obstacle, `rail_east_post_far` (x 2545–2593, y 776–824), carries the front ground down to 824 across the far post's columns, because its foot is at 812 and the front band began at 816 — four pixels is not a margin; twelve is. The near post's foot is 786 and the band already starts 30 px below it.

The floor that remains, in the rail's span: **`mud_near_2` x 2236–2545 · `mud_near_4` x 2545–2593 (y 824) · `mud_near_3` x 2593–2652**, continuous with `mud_near_5` west of them and `mud_near_1` east, so the front side is unbroken along the whole rail. Past either end the street is ordinary street at every depth — `mud_far_0`/`mud_mid_0` to the west, `mud_far_1`/`mud_mid_1` to the east. The boardwalk (y 548–598) is untouched: it is the town's far pavement, it carries exits and hotspots, and a man on it stands 40 px above the bar's top with no part of him drawn against the rail.

**THE POINTER NEEDED A RULE OF ITS OWN.** Carving is not enough. A click on the bar is a click on no box, and `WalkBoxes.nearest` answers with whatever box is closest in pixels — which, behind a rail standing just below the boardwalk, is **the boardwalk**. Clicking the fence would have sent him round the block to the far pavement. So the room declares `navigation.frontOnly` — the dead region and the row a destination inside it resolves below — and `WalkBoxes.target()` restricts a **destination** in that region to boxes at or below y 816. Routing, adjacency and the depth a figure is drawn at are untouched; `nearest` itself is deliberately unchanged, because it also answers *what plane is a figure standing here drawn at*, and the pie woman standing behind the rail must keep being masked by it.

**TWO CONSEQUENCES, both forced, both taken rather than worked around.** The saloon's door is dead centre of the rail's span, so every point in the mud in front of it was behind the rail: its approach point and its arrival move **up onto the boardwalk** (y 610 and 620 → 592, x untouched). That is where they belonged — you walk into a saloon off its pavement, with the rail in the street behind you. And the pie woman stands behind the rail on ground that is no longer a box, so her **placement** declares its own clip plane, exactly as Room 5's Winnie does behind her counter. Her position and her art are untouched. Her `approachRadius` is 40 and the nearest ground Thad can now reach is 110 px away, so her bark cannot fire where she stands: **MARKED FOR PHASE 2 RESTAGING**, per the ruling that an actor zone is not preserved at the expense of the rail rule.

**OCCLUSION, SIMPLIFIED ONLY WHERE SAFE.** The two rear-side proof points (2440,740 and 2470,706) are deleted with the ground they stood on — a proof point nobody can reach measures nothing. Plane 1 keeps its assertion at 2100,700, behind the trough, which is still walkable. **The plane-1 mask is deliberately not re-cut**: one file masks both the rail and the frozen trough, and re-cutting a shared mask to delete pixels that now mask nobody would risk the frozen half for no gain.

**THE TESTS.** `tools/gauntlet/rail-front-only.mjs` clicks the way a player clicks — the ground behind the rail six times, the bar, both posts, past both ends, five positions along the front, and the trough — and samples his feet every 120 ms for every walk, asserting he is never inside the rear region and never in a post's own columns above its foot. It replaces `rail-crossing.mjs`, which is **retired**: under this ruling there is nothing to cross, so that test could only have passed vacuously, and a check that cannot fail is worse than no check. `tests/front-only-rail.test.ts` asserts the same rule as geometry, without a browser: no walk box lies behind the rail, the front is unbroken and reachable end to end, both posts are clear, every click in the region resolves in front and not to the boardwalk, `nearest` is still unrestricted, the trough is reachable, and the doorway inside the span still works — found by asking the room which exit that is, not by naming it.

**NEGATIVE CONTROL, both halves.** Against the complete 1.5H build: **31 failures, nine of nine clicks behind or on the rail**. Against 1.5H's geometry with 1.5I's pointer rule: **2 failures** — he stops on the far post at 2568,816 (no notch) and a walk past the east end goes through the rear region (the ground is still there). Each half of the fix is load-bearing, and the test fails without either.

**Also fixed, because this change found it.** `check-staging` tested arrival points against `walkable` — the pre-carve bands — in every room, including rooms where `walkBoxes` replace them. It therefore called a point on Main Street's boardwalk "not on floor", and could not see obstacles at all: a mark inside the water trough passed. It now reads the boxes where a room has them.

**Frozen.** The plate, the chapel, the notice board, the company sign, the trough and its mask, the rail art and its occlusion mask, the dog, the lighting, the buildings, the mud, the boardwalk — untouched, byte for byte. **0 image operations.** No character moved or recast. The Nugget: OWNER-ACCEPTED AND UNCHANGED.
