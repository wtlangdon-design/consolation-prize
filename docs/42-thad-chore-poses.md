...
**Give the generator a NUMBER, not an adjective.** "Leave a clear margin on all four sides" was ignored three poses running — `use_near` at x=654 of 655, `give_offer` at x=720 of 721, `alight_coach` at x=722 of 723, every one of them a hand exactly on the frame edge. "Leave at least twenty pixels of magenta between the figure and every edge" produced 13px of clearance on the first attempt. Same for size: "roughly two and a half times taller than he is wide" was reinvented repeatedly, while "at least 1500 pixels tall" landed.

**The old margin clause, for the record.** Two poses in a row put the reaching hand exactly on the frame edge — `use_near` at x=654 of 655, `give_offer` at x=720 of 721 — despite the prompt asking for a clear margin on all four sides. The generator crops to the figure and ignores it. Nothing has been lost yet, but there is no headroom, and a pose reaching further would lose fingers. Check the margin on every reaching pose rather than trusting the clause.
