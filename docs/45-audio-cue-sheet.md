...
| `R05-FLOOR-STEP` | Thad physically loads/crosses the proud floorboard (the feet baseline swept across the plank; once per loading, re-armed when the foot leaves; never on hover, select, proximity, room load, time or idle) | Carry current `R05-M1` or `R05-M2` | SFX: one loose wooden-board creak synchronized to the board's depression; no music change | Cold, on the step | Natural short tail | Current room state | No |

> `R05-FLOOR-STEP` added under Tyler's authorization of 2026-09-05 (ROOM 5 FLOORBOARD — PROUD BOARD + PHYSICAL CREAK ADVERTISEMENT). Distinct from `R05-FLOOR`, the LISTEN cue. The runtime fires the cue by id (`engine/core/StepTriggers.ts`, logged by the probe) and shows the world caption CREEEAK; the audible asset and its playback are DEFERRED audio work, since the build carries no SFX layer (`engine/core/Music.ts`). Nothing else in this sheet changed.
