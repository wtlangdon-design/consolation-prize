...
| **6 · VISUAL / RUNTIME PROOF** | **see below** | Four-panel proof `renders/proofs/assay-office/` (contact sheet, proof.json, index.html); life proof `renders/proofs/assay-office/life/` (life.json with a one-second trace, contact sheet); style sheet `renders/room-05-style-continuity.png` + `.json` |
| **7 · REGRESSION** | **see below** | typecheck, `npm run validate`, `npm test`, `npm run gauntlet`, Room 1 proof — run on the committed tree at the end of this report |
...
- Full-suite reruns after the first final run: counted below.
