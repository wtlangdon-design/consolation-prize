...
**Correction 2 (same day, Tyler's visual review of correction 1):** contact had been proved by coordinates, not by silhouette — hand bottoms on the page's near edge, the writing fist over the ledger's corner, forearms sliced by the far-edge matte. Now the working forearms/hands/pen are a depth class above the surface and never cut; each hand is lifted a few whole pixels so paper shows around it; the pen shaft alone is re-angled 17° about its grip so the nib sits inside the writable page (measured off the day plate); and she stands 25 px further right (x1035, anchor y401), because the ledger is drawn 46 px right of her old centre and swinging the fist about the elbow far enough to reach it destroyed its pixels. Evidence: `renders/room-05-winnie-writing-before-current-corrected.png`, `renders/room-05-winnie-writing-contact-sheet.png`, `proofs/room-05/winnie-pen-state.json`; four-panel proof PASS clean on `50a6632` (lowest drawn row y395 in the page band y392–403; stand base y406), life proof PASS clean on `0150b0a7`. Tyler's visual review is the acceptance gate.

## NIGHT VISUAL CANDIDATE (2026-09-04, the final pilot image operation)

Tyler's night pass, after his review of the day candidate and the Winnie corrections. One image operation (8 of 8): the accepted DAY source edited to night with every approved night ancestor transmitted (Room 1 live frame and plate, the Main Street night plate, the Nugget, Thad, the casting masters). Source `art/staging/room-05/plate-source-03-night.png` (5ea39bd8201e); safe frame SAFE, same crop and Lanczos-3; candidate `art/staging/room-05/plate-03-night/candidate-1920x864.png` (72124eb6f213). Geometry drift against the DAY candidate: every annotated rect at (0,0), PASS (`proofs/room-05/night-geometry-drift.json`). Winnie's approved sheet and the stand relit by the existing match-local pass with byte-identical alpha (c116e34e8cdb, 12225c416c9d). Loaded through the candidate override, three swaps, nothing in content changed. NIGHT four-panel proof PASS clean on `015a165f` (`renders/proofs/assay-office-night/`); short night Winnie route PASS clean on `75f9e0bc` (`renders/proofs/assay-office-night/winnie/`); night style sheet `renders/room-05-night-style-continuity.png`; night states `renders/room-05-night-winnie-states.png`; regression numbers `proofs/room-05/night-winnie-regression.json`. The DAY candidate is preserved as ROOM 5 — DAY VISUAL CANDIDATE. Nothing promoted; `visual_accepted` unset.

**Lighting balance (same day, no image operation):** Tyler's review found the night interior too dark to write by. `tools/grade/lift-work-area.py` — a deterministic, hue-preserving, feathered work-area lift (0.17) plus the stove's radial bounce (0.10), exterior glazing excluded, corners unchanged — gives `art/staging/room-05/plate-03-night-lift/candidate-1920x864.png` (92e3f8f5b70a): hands and ledger +22%, counter front +23%, face +22%, scales +18%, bottles +20%, corners and glazing 0–2% (`plate-03-night-lift/grade.json`). Winnie and the stand relit against it (d0b138fc6f18, 6d40775eaf42). Comparison `renders/room-05-night-lift-before-after.png`; identity proof `renders/proofs/assay-office-night-lift/`. The ungraded night candidate is kept.

## THE HANGING WORK LAMP (post-pilot owner visual revision, 2026-09-04)

Tyler's review of the night candidate: the stove alone does not explain a clerk writing at night. Owner ruling: one small plain period oil lamp over the ledger is canon, and ONE additional image operation is authorized for it, accounted apart from the pilot — **autonomous pilot 8/8; post-pilot owner visual revision 1/1**. The lamp was acquired in context (the night source edited to add exactly one lamp; nine references transmitted; `art/staging/room-05/lamp-source-01.png`, cc76a6bfaf60) and cut out as a prop (`lamp-01/hanging-lamp-overlay.png`, 5575e30126f2) drawn over the unchanged DAY and NIGHT plates as THE HANGING LAMP's `lit` state image; its light is a room lamp, negligible by day and the principal work light by night (`amountByState`, dev-only `?state=night`). Geometry drift of the edit against the night candidate: every rect at (0,0). Canon: doc 25 and doc 05 (Tyler's six lines). Winnie and the stand relit under the lamp with identical alpha. Proofs: DAY four-panel PASS clean on `ffaf868` (`renders/proofs/assay-office-lamp-day/`), NIGHT four-panel PASS clean on `99af681e` with state night (`renders/proofs/assay-office-lamp-night/`), night route PASS clean on `73a3b31c` (`.../winnie/`): the fixture matches its overlay in every capture, pen state correct where the stand is visible, contact band held. Evidence: `renders/room-05-lamp-day-night.png`, `renders/room-05-lamp-night-winnie-states.png`, `proofs/room-05/lamp-placement.json`, `proofs/room-05/lamp-night-winnie-regression.json`, `art/staging/room-05/LAMP.md`. The recording step of the driver threw after the API had returned (a role check); the ledger row was written by hand from the same request data minutes later and says so, including that the response's token usage was not captured. Nothing promoted; `visual_accepted` unset.
